...
- Для тестов: при `EMULATOR_MANUAL=true` фоновый цикл не запускается, а ручка
  `POST /internal/emulator/tick?advance_sec=N` сдвигает «виртуальное время» и выполняет один
  проход. Без этого флага ручка не регистрируется. Наружу не публикуется (нет в ingress).
...
- Доставка at-least-once; клиент идемпотентен (по событию перезапрашивает данные).
  Порядок — по `seq`; BFF отбрасывает `seq` ≤ последнего отправленного в сокет.
