...
- Режим эмулятора виден снаружи: `GET /health` service-core отдаёт `emulator_manual` и
  `emulator_enabled` (план 08). Поля диагностические — на `ok` они не влияют, оба режима
  штатные. Смысл в том, что неверный режим обнаруживается до первого теста, а не как `404`
  на `tick` посреди прогона (за план 07 на это наступили дважды). Ручной режим задаётся
  **файлом** `Backend/docker-compose.e2e.yml`, а не префиксом команды: `environment` в compose
  — желаемое состояние, и любой `up` того же проекта без этого префикса молча пересоздавал
  service-core в автоматическом режиме.

**Решение (план 08): e2e не являются частью проверки кластерного деплоя.**
`POST /internal/emulator/tick` не публикуется наружу ни в одном окружении: в compose её нет
в Traefik, в k3s к service-core по NetworkPolicy ходит только BFF, а BFF её не проксирует.
Из двух возможных выходов — (a) e2e живут только на compose-стенде, (b) в k3s для них
открывается отдельный путь (port-forward в джобе) — выбран **(a)**. Открывать ручку изнутри
кластера ради тестов значит держать в проде дверь, существующую только для тестов, и ставить
зелёный деплой в зависимость от неё; цена варианта (b) — постоянная, польза — разовая.

Следствие, которое нельзя подразумевать: **Playwright-сюит (`Frontend/e2e`) не проверяет
кластерную конфигурацию вообще.** Он гоняется на compose-стенде (`make e2e-stand-up`) и
проверяет прикладной сквозной сценарий — регистрацию, сделку, события из Redis на экране.
Кластерный деплой проверяется другим и полностью:

- `helm lint` и `helm template` на всех пяти релизах — рендер манифестов, включая ingress,
  NetworkPolicy и Secret (единственная проверка, доступная без кластера, — см. §8);
- пробы `/health` и `/ready` каждого сервиса (readiness BFF учитывает живую подписку Redis
  и доступность auth/core/commission) — `helm upgrade --wait` без них ничего не значит;
- smoke-цели `make k3s-test-*` — они ходят в кластер снаружи, через тот же периметр, что и
  пользователь (только `frontend` и `bff`), и потому проверяют именно опубликованную
  конфигурацию, а не обходят её.

Если когда-нибудь потребуется гонять e2e против кластера, это отдельное решение с отдельной
ценой (вариант (b)), а не «дописать URL в переменную».
