...
### Task 5: Ответ раньше коммита — чинить во всех четырёх местах

Задача добавлена по ходу плана: диагностика находки N6 в Task 1 вскрыла дефект на
главном пути демонстрации. Его не было видно раньше, потому что smoke-цели возвращали
нулевой код возврата на любой ответ и прятали мигающий 401.

**Files:**
- Modify: `services/service-auth/src/auth/register.cc`, `services/service-core/src/repository.cc` (`create`, `apply`), `services/service-core/src/outbox_publisher.cc`
- Create: обёртка ожидания коммита (заголовок в общем месте C++-сервисов — куда именно, решает исполнитель по структуре репозитория; она нужна обоим сервисам)
- Test: `tests/api/test_auth_commit.py` (новый), плюс расширение `tests/api/test_core_deals.py`

**Interfaces:**
- Consumes: Drogon `DbClient`/`Transaction`.
- Produces: HTTP-ответ уходит клиенту только после подтверждённого `COMMIT`; неудачный коммит становится `500 INTERNAL_ERROR`, а не «успехом» с потерянными данными.

**Суть дефекта.** В `register.cc` транзакция создаётся на строке 39, а `cb(...)` с 200 и
токеном вызывается на строке 76 — когда `tx` ещё жива. У Drogon нет публичного
`commit()` (в `drogon/orm/DbClient.h` он закомментирован; наружу только `rollback()` и
`setCommitCallback()`), поэтому `COMMIT` уходит в деструкторе `Transaction`, то есть уже
после записи ответа в сокет. Клиент получает `user_id` и токен, пока COMMIT в полёте;
следующий запрос берёт из пула другое соединение и под READ COMMITTED не видит ни
пользователя, ни компанию. Измеренное окно — миллисекунды: прямой `curl --next` без паузы
даёт 401 в 20 случаях из 20, с паузой ~200 мс — 0 из 12.

Тот же паттерн в `repository.cc`: `create()` возвращает `DealDetail` при живой `trans`,
и ответ «сделка создана» может обогнать её коммит. Это ровно тот же класс, и правка
обязана покрыть все четыре места, а не одно найденное.

- [ ] **Step 1: Воспроизвести до починки**

Написать `tests/api/test_auth_commit.py`: регистрация, затем **немедленно**, без пауз и
без ретраев, `POST /login` теми же кредами и `GET /me` с выданным токеном. Оба должны
дать 200. Тест обязан падать на текущем коде — если он зелёный с первого запуска, значит
он не воспроизводит дефект, и чинить пока нечего: разберись, почему, прежде чем трогать
`register.cc`.

Прогнать его несколько раз подряд (окно измеряется миллисекундами, одиночный прогон
ничего не доказывает) и записать в отчёт, сколько прогонов из скольких упало.

- [ ] **Step 2: Обёртка ожидания коммита**

Сделать awaitable-обёртку над `setCommitCallback`: она вешает колбэк, отпускает последнюю
ссылку на транзакцию (это и запускает `COMMIT`) и ждёт результат. Колбэк отдаёт признак
успеха — **неуспех обязан приводить к `500 INTERNAL_ERROR`**, а не молча считаться
коммитом. Обёртка нужна и auth, и core, поэтому живёт в общем месте, а не копируется.

Подводные камни, которые надо обработать явно, а не «по месту»: колбэк может выполниться
на другом треде цикла событий; транзакция не должна удерживаться ссылкой из самого
колбэка (иначе деструктор не вызовется и ожидание зависнет); путь с `rollback()`
(ветка `USER_EXISTS`) не должен ждать коммита, которого не будет.

- [ ] **Step 3: Применить во всех четырёх местах**

`register.cc` (ответ — после подтверждённого коммита), `repository.cc::create`,
`repository.cc::apply`, `outbox_publisher.cc`. Для `outbox_publisher.cc` решить и записать
в отчёте, нужна ли там та же гарантия: публикация в Redis после неподтверждённого коммита
означает событие о сделке, которой в базе может не оказаться.

Ветки ошибок не трогать по инерции: где сейчас `rollback()` и ответ 4xx — там поведение
остаётся, меняется только успешный путь.

- [ ] **Step 4: Доказать починку**

Тест из Step 1 зелёный **на серии прогонов, а не на одном**. Плюс прямой `curl --next` без
паузы (тот самый сценарий, что давал 401 в 20 из 20) — зелёный. В отчёте привести числа
до и после.

- [ ] **Step 5: Регрессия на стороне сделок**

Расширить `tests/api/test_core_deals.py`: создать сделку и **немедленно** запросить её по
`GET /api/deals/{id}` без пауз и ретраев. Тот же дефект, тот же способ доказательства.

- [ ] **Step 6: Полный прогон**

`make test-cpp`, `make test-api`, `make test-commission`, плюс smoke-цели, которые теперь
честно краснеют. e2e прогнать обязательно: правка лежит ровно на их пути.

- [ ] **Step 7: Commit**

```bash
git add -A && git commit -m "fix(auth,core): send response only after the transaction actually commits"
```
