...
### Состояние на 2026-09-21 и что осталось сделать руками

Оба репозитория запушены (`IVR-CBDC/Backend`, `IVR-CBDC/Frontend`), защита
ветки `main` включена в обоих, образы публикуются. Проверено прогонами в
настоящем CI, а не рассуждением:

| Шаг | Состояние |
|---|---|
| Backend CI: shellcheck, helm, cpp, commission, api | **проходит** |
| Backend: сборка и публикация 3 образов | **проходит** (`sha-<7>` и `:main`) |
| Frontend CI: `bff`, `frontend` | **проходит** |
| Frontend CI: `e2e` | **проходит** — собирает backend из исходников, если пакеты недоступны |
| Frontend: сборка и публикация bff + spa | **проходит** |
| Frontend → Backend: dispatch с тегами | **падает** — нет PAT (пункт 2) |
| Backend CD: выкат в k3s | **падает** — нужны пункты 1 и 3 |

Осталось три действия, которые нельзя выполнить из кода — только из
аккаунта владельца:

**1. Открыть доступ к пакетам ghcr.** Пакет, опубликованный workflow'ом,
приватен и привязан к своему репозиторию, поэтому CD Backend не видит
`frontend/bff` и `frontend/spa`, а CI фронта — backend-образов. Для каждого
из пяти пакетов (Packages в профиле организации → пакет → Package settings):
сделать **Public**, либо в *Manage Actions access* добавить второй
репозиторий с ролью `Read`. Для демо проще первое.

**Фронтовый CI на это больше не завязан.** Джоба `e2e` пробует стянуть
backend-образы, а если не может — собирает их из исходников (`./backend` уже
в чекауте, репозиторий публичный). Медленнее, зато зелено без действий в UI;
как только доступ появится, шаг сам вернётся на быстрый путь. Ускорить можно
секретом `GHCR_READ_TOKEN` (PAT с `read:packages`) в `IVR-CBDC/Frontend`.

А вот **CD Backend обойти нечем**: ему нужны именно образы фронта, собрать
он их не может. Так что для выката пункт 1 обязателен.

**2. `BACKEND_DISPATCH_TOKEN`** — PAT с правом `contents: write` на
`IVR-CBDC/Backend`, положить в секреты `IVR-CBDC/Frontend`. Без него джоба
`dispatch` падает с явным сообщением, а теги в `frontend-tags.env`
приходится править руками (так и сделано сейчас — значение `sha-b076456`
подставлено вручную; именно его прислал бы dispatch).

**3. Секреты паролей БД** для CD: `DB_PASSWORD_AUTH`, `DB_PASSWORD_CORE`,
`DB_PASSWORD_COMMISSION` в секретах `IVR-CBDC/Backend`. Пароль обязан
состоять только из `A-Za-z0-9` и `-._~!$&'()*+,;=` — он уезжает в DSN, то
есть в URL, и `infra/k3s-preflight.sh` отвергнет остальное. **`openssl rand
-base64` не годится — даёт `/`.** Годится, например:
`openssl rand -hex 24`.

Порядок отказов в CD выстроен так, что каждая следующая преграда указывает
на настоящую причину: сперва формат тега, потом наличие образа, потом
пароли — и только затем первый `helm upgrade`.
