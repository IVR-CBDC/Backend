...
| Frontend → Backend: dispatch с тегами | **падает** — нет PAT |
| Backend CD: проверка образов | **падает** — фронтовые пакеты недоступны |
...
Альтернатива для фронтового CI, если оставлять пакеты приватными: секрет
`GHCR_READ_TOKEN` (PAT с `read:packages`) в `IVR-CBDC/Frontend` — джоба
`e2e` его уже поддерживает и сама называет в сообщении об ошибке. Для CD
Backend такого обходного пути нет: там нужен доступ к пакету.
